...
const Body = struct {
    ...
    pub const control_ir: cir.Program = .{
        .label = "kernel-wasm-machine-failure-vector",
...
const Program = program_v2.program("kernel-wasm-machine-failure-vector", Body);
const options: machine.Options = .{
    .maximum_frames = 4,
    .maximum_state_bytes = 4096,
    .maximum_machine_fuel = 0,
};
...
pub fn main(init: std.process.Init) !void {
    ...
    const failure = switch (try kernel_v1.step(
        image,
        ...
        .machine_failed => |failed| failed,
        else => return error.UnexpectedOutcome,
    };
    if (failure.failure != .execution_budget_exceeded) {
        return error.UnexpectedOutcome;
    }
    var expected: [40 + 4096]u8 = [_]u8{0} ** (40 + 4096);
    @memcpy(expected[0..8], "ABL_KOU1");
    writeInt(u16, &expected, 8, 1);
    writeInt(u16, &expected, 10, 4);
    writeInt(u16, &expected, 12, 7);
    writeInt(u64, &expected, 16, fuel);
    writeInt(u32, &expected, 24, failure.state.len);
    @memcpy(expected[40..][0..failure.state.len], failure.state);
    const expected_length = 40 + failure.state.len;

    var stdout_buffer: [4096]u8 = undefined;
    var stdout_writer = std.Io.File.stdout().writer(init.io, &stdout_buffer);
    const stdout = &stdout_writer.interface;
    try stdout.writeInt(u32, @intCast(input_length), .little);
    try stdout.writeInt(u32, @intCast(expected_length), .little);
    try stdout.writeAll(input[0..input_length]);
    try stdout.writeAll(expected[0..expected_length]);
    try stdout.flush();
}
