...
fn plan() boundary.ir.ProgramPlan {
    ...
    const instructions = [_]boundary.ir.plan.Instruction{
        ...
        .{ .kind = .add_i32, .dst = source.index, .operand = source.index, .aux = zero.index },
        .{ .kind = .compare_eq_zero, .dst = condition.index, .operand = source.index },
    };
    ...
    return boundary.ir.builder.finish(.{
        .label = "static-machine-in-place-add-i32-predicate",
        .ir_hash = 1,
        .entry = root,
        .functions = &functions,
        .requirements = &.{},
...
const Program = boundary.program("static-machine-in-place-add-i32-predicate", struct {}, Body);
