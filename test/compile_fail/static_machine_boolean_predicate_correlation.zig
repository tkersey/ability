...
fn plan() boundary.ir.ProgramPlan {
    ...
    const instructions = [_]boundary.ir.plan.Instruction{
        .{ .kind = .compare_eq_zero, .dst = derived.index, .operand = source.index },
    ...
    return boundary.ir.builder.finish(.{
        .label = "static-machine-boolean-predicate-correlation",
        .ir_hash = 1,
        .entry = root,
        .functions = &functions,
        .requirements = &.{},
        .ops = &.{},
        .outputs = &.{},
        .locals = &.{ .{ .codec = .i32 }, .{ .codec = .bool }, .{ .codec = .bool } },
        .blocks = &blocks,
...
const Program = boundary.program("static-machine-boolean-predicate-correlation", struct {}, Body);
