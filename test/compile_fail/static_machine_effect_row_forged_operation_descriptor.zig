...
fn plan() boundary.ir.ProgramPlan {
    ...
    return boundary.ir.builder.finish(.{
        .label = "static-machine-effect-row-forged-operation-descriptor",
        .ir_hash = 1,
        .entry = root,
        .functions = &functions,
...
const Program = boundary.program("static-machine-effect-row-forged-operation-descriptor", struct {}, Body);
const Machine = boundary.staticMachine(Program, .{});
const RealSite = Machine.EffectRow.operationSite("protocol", "step", 0);
...
comptime {
    Machine.EffectRow.assertOperationSitesCovered(.{ForgedSite});
}
