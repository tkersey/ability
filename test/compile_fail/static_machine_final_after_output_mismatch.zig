...
fn plan() boundary.ir.ProgramPlan {
    ...
    return boundary.ir.builder.finish(.{
        .label = "static-machine-final-after-output-mismatch",
        .ir_hash = 1,
        .entry = root,
        .functions = &functions,
...
const Handlers = struct {
    step: struct {
        ...
        pub fn afterDispatch(_: *const @This(), value: i32) error{}!bool {
            return value != 0;
        }
    },
};
...
const Program = boundary.program("static-machine-final-after-output-mismatch", Handlers, Body);
const Machine = boundary.staticMachine(Program, .{});

test "StaticMachine rejects a potentially final after output mismatch" {
    _ = Machine;
}
