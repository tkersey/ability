...
fn plan() boundary.ir.ProgramPlan {
    ...
    return boundary.ir.builder.finish(.{
        .label = "static-machine-after-chain-mismatch",
        .ir_hash = 1,
        .entry = root,
        .functions = &functions,
...
const Program = boundary.program("static-machine-after-chain-mismatch", Handlers, Body);
const Machine = boundary.staticMachine(Program, .{});

test "StaticMachine rejects an incompatible reachable after-handler chain" {
    _ = Machine;
}
