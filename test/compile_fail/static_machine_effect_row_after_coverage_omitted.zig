...
const Body = struct {
    pub const compiled_plan = plan("static-machine-effect-row-after-coverage-omitted");
};
const Program = boundary.program("static-machine-effect-row-after-coverage-omitted", struct {}, Body);
const Machine = boundary.staticMachine(Program, .{});
const Operation = Program.protocol.operationSite("protocol", "step", 0);
const OperationHandler = struct {
    fn handle(
        _: anytype,
        _: anytype,
        _: Program.Handler.Control,
    ) Program.Handler.Outcome(Operation) {
        return Program.Handler.@"resume"(Operation, @as(i32, 1));
    }
};
const Interpreter = Program.Interpreter(.{
    Program.Handler.operation(Operation, OperationHandler.handle),
});
