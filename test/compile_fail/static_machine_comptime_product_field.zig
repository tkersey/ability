...
const Payload = struct {
    comptime protocol_version: u64 = 1,
    value: u64,
};
...
fn plan() boundary.ir.ProgramPlan {
    ...
    return boundary.ir.builder.finish(.{
        .label = "static-machine-comptime-product-field",
        .ir_hash = 1,
        .entry = root,
        .functions = &functions,
        .requirements = &.{},
...
const Program = boundary.program("static-machine-comptime-product-field", struct {}, Body);
const Machine = boundary.staticMachine(Program, .{});

test "StaticMachine rejects comptime product fields" {
    _ = Machine;
}
