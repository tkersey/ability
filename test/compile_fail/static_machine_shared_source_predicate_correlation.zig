...
fn plan() boundary.ir.ProgramPlan {
    ...
    const instructions = [_]boundary.ir.plan.Instruction{
        .{ .kind = .add_const_i32, .dst = first.index, .operand = source.index, .aux = 0 },
        .{ .kind = .add_const_i32, .dst = second.index, .operand = source.index, .aux = 0 },
    ...
    return boundary.ir.builder.finish(.{
        .label = "static-machine-shared-source-predicate-correlation",
        .ir_hash = 1,
        .entry = root,
        .functions = &functions,
        .requirements = &.{},
...
const Program = boundary.program("static-machine-shared-source-predicate-correlation", struct {}, Body);
