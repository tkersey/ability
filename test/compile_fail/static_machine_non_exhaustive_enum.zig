...
fn plan() boundary.ir.ProgramPlan {
    ...
    return boundary.ir.builder.finish(.{
        .label = "static-machine-non-exhaustive-enum",
        .ir_hash = 1,
        .entry = root,
        .functions = &functions,
        .requirements = &.{},
...
const Program = boundary.program("static-machine-non-exhaustive-enum", struct {}, Body);
const Machine = boundary.staticMachine(Program, .{});

test "StaticMachine rejects non-exhaustive enum carriers" {
    _ = Machine;
}
