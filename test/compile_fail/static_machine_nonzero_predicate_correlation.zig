...
fn plan() boundary.ir.ProgramPlan {
    ...
    const instructions = [_]boundary.ir.plan.Instruction{
        .{ .kind = .add_const_i32, .dst = derived.index, .operand = source.index, .aux = 1 },
    ...
    return boundary.ir.builder.finish(.{
        .label = "static-machine-nonzero-predicate-correlation",
        .ir_hash = 1,
        .entry = root,
        .functions = &functions,
        .requirements = &.{},
...
const Program = boundary.program("static-machine-nonzero-predicate-correlation", struct {}, Body);
