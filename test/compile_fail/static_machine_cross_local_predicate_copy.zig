...
fn plan() boundary.ir.ProgramPlan {
    ...
    const instructions = [_]boundary.ir.plan.Instruction{
        ...
        .{ .kind = .add_const_i32, .dst = copy.index, .operand = source.index, .aux = 0 },
    ...
    return boundary.ir.builder.finish(.{
        .label = "static-machine-cross-local-predicate-copy",
        .ir_hash = 1,
        .entry = root,
        .functions = &functions,
        .requirements = &.{},
...
const Program = boundary.program("static-machine-cross-local-predicate-copy", struct {}, Body);
