...
fn plan() boundary.ir.ProgramPlan {
    ...
    const instructions = [_]boundary.ir.plan.Instruction{
        ...
        .{ .kind = .compare_eq_zero, .dst = condition.index, .operand = condition.index },
        .{ .kind = .const_i32, .dst = inner_resume.index, .operand = 0 },
    ...
    const blocks = [_]boundary.ir.plan.Block{
        ...
        .{ .first_instruction = 5, .instruction_count = 2, .terminator_index = 5 },
    };
    ...
    return boundary.ir.builder.finish(.{
        .label = "static-machine-in-place-after-chain-mismatch",
        .ir_hash = 1,
        .entry = root,
        .functions = &functions,
        ...
        .ops = &ops,
        .outputs = &.{},
        .locals = &.{ .{ .codec = .bool }, .{ .codec = .i32 }, .{ .codec = .i32 } },
        .blocks = &blocks,
...
const Program = boundary.program("static-machine-in-place-after-chain-mismatch", Handlers, Body);
const Machine = boundary.staticMachine(Program, .{});

test "StaticMachine invalidates predicate correlation after an in-place comparison" {
    _ = Machine;
}
