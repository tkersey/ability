...
fn plan() boundary.ir.ProgramPlan {
    ...
    return boundary.ir.builder.finish(.{
        .label = "static-machine-reserved-program-contract-violation",
        .ir_hash = 1,
        .entry = root,
        .functions = &functions,
        .requirements = &.{},
...
const Body = struct {
    pub const Error = error{ProgramContractViolation};
    pub const compiled_plan = plan();
};
const Program = boundary.program("static-machine-reserved-program-contract-violation", struct {}, Body);
const Machine = boundary.staticMachine(Program, .{});

test "StaticMachine reserves ProgramContractViolation for operational failure" {
    _ = Machine;
}
