// zlinter-disable declaration_naming field_naming field_ordering require_doc_comment no_swallow_error
const boundary = @import("boundary");

const Status = enum(c_long) {
    ready = 1,
    waiting = 2,
};
...
fn plan() boundary.ir.ProgramPlan {
    ...
    return boundary.ir.builder.finish(.{
        .label = "static-machine-c-abi-enum-tag",
        .ir_hash = 1,
        .entry = root,
        .functions = &functions,
        .requirements = &.{},
...
const Program = boundary.program("static-machine-c-abi-enum-tag", struct {}, Body);
const Machine = boundary.staticMachine(Program, .{});

test "StaticMachine rejects C-ABI enum tag types" {
    _ = Machine;
}
