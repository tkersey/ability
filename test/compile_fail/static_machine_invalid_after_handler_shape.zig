...
fn plan() boundary.ir.ProgramPlan {
    ...
    return boundary.ir.builder.finish(.{
        .label = "static-machine-invalid-after-handler-shape",
        .ir_hash = 1,
        .entry = root,
        .functions = &functions,
...
const Handlers = struct {
    @"test": struct {
        ...
        pub fn afterDispatch(_: *const @This()) error{}!i32 {
            return 1;
        }
    },
};
...
const Program = boundary.program("static-machine-invalid-after-handler-shape", Handlers, Body);
const Machine = boundary.staticMachine(Program, .{});

test "StaticMachine rejects invalid after handler runtime shape" {
    _ = Machine;
}
