...
fn plan() boundary.ir.ProgramPlan {
    ...
    const one = boundary.ir.builder.local(root, 1);
    const derived = boundary.ir.builder.local(root, 2);
    const condition = boundary.ir.builder.local(root, 3);
    const instructions = [_]boundary.ir.plan.Instruction{
        .{ .kind = .const_i32, .dst = one.index, .operand = 1 },
        .{ .kind = .add_i32, .dst = derived.index, .operand = source.index, .aux = one.index },
    ...
    return boundary.ir.builder.finish(.{
        .label = "static-machine-add-i32-predicate-correlation",
        .ir_hash = 1,
        .entry = root,
        .functions = &functions,
        .requirements = &.{},
...
const Program = boundary.program("static-machine-add-i32-predicate-correlation", struct {}, Body);
