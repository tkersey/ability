...
the truth of every predicate that uniquely gates each operation, helper, or
nested-provider suspension site. Canonical pending-state validation checks
every such truth against the decoded source local in every parked frame, so a
later predicate cannot erase an earlier gate. Construction rejects a suspension
site when no predicate-consistent path can reach it.
